...
[ApiController]
[Route("api/master-data/boms")]
[Authorize]
public class BomsController(IBomService service) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> GetList(
        ...
        => ApiResults.FromResult(await service.GetListAsync(
            new BomQueryRequest(productId, isActive, keyword, page, pageSize), cancellationToken));
    ...
    public async Task<IActionResult> Create([FromBody] CreateBomRequest request, CancellationToken cancellationToken)
        => ApiResults.FromResult(await service.CreateAsync(request, cancellationToken));

    [HttpPut("{id:guid}")]
    [HasPermission(Permissions.MasterData.Manage)]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateBomRequest request, CancellationToken cancellationToken)
