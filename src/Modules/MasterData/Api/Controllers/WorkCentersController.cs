...
/// <summary>工作中心主数据（产线 / 单元 / 工位 / 设备）。</summary>
[ApiController]
[Route("api/master-data/work-centers")]
[Authorize]
public class WorkCentersController(IWorkCenterService service) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> Create([FromBody] CreateWorkCenterRequest request, CancellationToken cancellationToken)
        => ApiResults.FromResult(await service.CreateAsync(request, cancellationToken));

    [HttpPut("{id:guid}")]
    [HasPermission(Permissions.MasterData.Manage)]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateWorkCenterRequest request, CancellationToken cancellationToken)
