...
**Q：提问中切到别的页面，回来之后记录还在吗？**
在。会话状态（提问记录 / 输入草稿 / 「分析中」状态）放在 Pinia store 里（`stores/assistant.js`），
**请求生命周期也归 store** —— 所以切页面不会中断这次查询，回来就能看到它的结果，
按钮的「分析中…」状态也是连续的，顺带防住了跨页面重复提交。
提问记录还会缓存到 `sessionStorage`：**刷新（F5）也能接上**，标签页关掉即清；
登录 / 退出会主动清空，避免下一个用户看到上一个用户的查询结果。

> 注意：其它页面**没有**这层保护 —— 本应用没有用 `keep-alive`，路由一切走组件就卸载，
> 列表页的筛选条件、翻页位置切走一样会丢。要不要给列表页统一加 `keep-alive`
> 属于应用级取舍（保住状态 vs 保证回来看到最新数据），需要的话可以单独做。
