...
- 🔒 **端口只绑回环**（2026-09-23 加固）：编排里是 `127.0.0.1:8091:8080`，不再对公网开放。
  容器之间走 `qiaomes-net` 用服务名 `ai-iteration` 访问，QiaoMES 不受影响。
- 🔒 **写接口全部要求管理员密钥**（2026-09-23 加固）：`POST /api/tasks`（建任务）与
  `POST /api/workspaces`（注册工作区）此前**免鉴权**，现在与 `lease` / `report` 一样过 `AdminKeyFilter`
  （未配密钥即 503 全禁）。只读接口（`GET /api/tasks/{id}`、`GET /api/cycles/current` 等）保持开放 ——
  它们不含凭据、也不改状态。
- ⚠️ **代价（要恢复自动执行时看这条）**：开发机上的 `AiIteration.Worker` 要靠公网连 8091 租任务，
  端口收回回环之后它**连不上了**（SSH 被公司网络阻断，也没有隧道可用）。要恢复自动执行，二选一：
  ① 经网关反代暴露（Caddy + HTTPS + 路径前缀，只放行 `POST /api/tasks/lease`、`/{id}/report` 两条路）；
  ② 临时把编排那行改回 `8091:8080`（用完改回来）。在此之前，自迭代停在「计划 + 审阅」这一段。
