...
**阶段 2 进行中**：主数据（2.1）全部完成 —— 产品/物料/工序/工作中心/BOM/工艺路线（含版本化：版本唯一、单一生效版本、生效版本不可删）；工单已支持**按生效工艺路线展开工序任务**（快照 BOM/工艺路线版本）与**工序级报工**（良品/不良/报废 + 不良代码，越序报工被拒绝，全部工序完成则工单自动完成）；剩余 **工时与设备维度、WIP 过站、返工返修、主数据导入导出**。
...
| 2.3 | 工单按工艺路线展开 | ① 下达自动生成工序任务 ✅（顺序、工作中心、工序编码/名称、标准工时、计划数量全部快照）；② 越序报工被拒绝（前序工序必须已完成）✅；③ 工单进度：工序任务的良品/不良/报废与进度百分比 ✅，工单完成数取**最后一道工序良品数**（加权进度视图待前端补充） | [~] |
| 2.4 | 工序级报工 | 工序 ✅、数量 ✅、良品/不良/报废 ✅、不良代码 ✅、操作员 ✅、开始/结束时间 ✅（工序任务上）；**工时回填、设备维度、返工返修待做** | [~] |
| 2.5 | 在制品（WIP）与过站 | ① SN/批次级过站记录；② 任意 SN 可查"当前工序、历史工序、操作人、设备、时间" | [ ] |
| 2.6 | 主数据管理界面 | 产品/物料/工序/工作中心/BOM/工艺路线 的 CRUD 页面全部完成 ✅（前四者配置驱动共用界面，BOM/工艺路线用含明细行编辑的专用表单）；工单页支持产品下拉选择与工序报工对话框 ✅；导入（Excel/CSV）导出待做 | [~] |
