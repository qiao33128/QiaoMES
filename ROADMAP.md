...
**阶段 2 进行中**：`MasterData` 模块已完成**产品 / 物料 / 工序 / 工作中心 / BOM / 工艺路线**全部主数据（含版本化规则：同产品内版本唯一、同一时刻只有一个生效版本、生效版本不允许删除）与前端维护界面；剩余 **工单按路线展开工序任务、工序级报工、WIP 过站**。
...
| 2.2 | BOM 与工艺路线版本化 | ① 同一产品多版本、仅一个生效版本 ✅（激活新版本时自动失效旧版本）；② 生效版本不允许删除 ✅；③ 版本号在产品内唯一 ✅；④ 工单下达时**快照**所用版本 —— 与 2.3 一起实现 | [~] |
...
| 2.6 | 主数据管理界面 | 产品/物料/工序/工作中心/BOM/工艺路线 的 CRUD 页面全部完成 ✅（前四者配置驱动共用界面，BOM/工艺路线用含明细行编辑的专用表单）；导入（Excel/CSV）导出待做 | [~] |
