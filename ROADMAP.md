...
**阶段 2 进行中**：`MasterData` 模块已完成**产品 / 物料 / 工序 / 工作中心**四种主数据的完整 CRUD（编码唯一、关键字分页、启停、权限约束）与配置驱动的前端维护界面；**BOM、工艺路线、工单按路线展开、工序级报工**待做。
...
| # | 任务 | 验收标准（DoD） | 状态 |
|---|---|---|---|
| 2.1 | `MasterData` 模块 | 产品（Product）✅、物料（Material）✅、工序（Operation）✅、工作中心（WorkCenter，覆盖产线/单元/工位/设备）✅ 已完成；**BOM、工艺路线（Routing/RoutingStep）待做** | [~] |
...
| 2.6 | 主数据管理界面 | 产品/物料/工序/工作中心 的统一 CRUD 页面已完成（配置驱动）✅；BOM/Routing 页面与导入（Excel/CSV）导出待做 | [~] |
