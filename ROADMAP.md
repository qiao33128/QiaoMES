...
| 日期 | 版本 | 变更 |
|---|---|---|
| 2026-09-23 | v3.9 | **两项安全加固收口（用户拍板「都弄掉」）**。① **生产 JWT 密钥不再是公开占位串**：此前服务器 `.env` 里存的正是那个写在 `.env.example` 与编排历史里的占位值（长度 58，实测核对过），等于任何人都能伪造令牌、而日志里看不出来。改法是**部署脚本自愈**——第 2 步发现 `.env` 里 `JWT_SECRET_KEY` 为空或是占位值（`QiaoMES_*` / 含 `Change_Me`）就当场换成随机串并把影响打进日志（一次性，之后不再动）。放在脚本里而不是 CI，是因为只有这里能同时看到服务器现值、又能安全改它。代价：所有人重新登录一次、问数页已存模型密钥需重填。② **自迭代服务的 8091 收回回环**（`127.0.0.1:8091:8080`），不再对公网开放；③ 同时把它的**免鉴权写接口**补上校验：`POST /api/tasks`（建任务）与 `POST /api/workspaces`（注册工作区）现在都过 `AdminKeyFilter`（AI迭代 仓 `ab5eb85`，镜像已重建推仓 `:latest` + `:ab5eb85`）—— 建任务会让有仓库凭据的 worker 真去改代码，注册工作区决定去哪 clone，这两条免鉴权比端口暴露本身更危险。只读接口保持开放。⚠️ 代价：开发机的 `AiIteration.Worker` 从此连不上（要恢复走网关反代或临时放开端口），自迭代暂时停在「计划 + 审阅」。 |
| 2026-09-23 | v3.8 | **开发环境从服务器迁回开发机**。缘由是一次真实事故：把 dev 栈也放在那台 2 vCPU / 896MB 的服务器上后，一次 dev 部署（拉镜像 + 容器重建 + `docker image prune`）把**磁盘 I/O 拖死** —— `load average` 冲到 38，而 `ps` 里没有任何进程在吃 CPU（**高负载 + 无人占 CPU = 等 I/O**，不是 OOM），HTTP 全超时、**生产一起不可达**，约 17 分钟后自愈。结论：这台机器给不了第二套环境。改法：① dev 环境改由 `tools/start-dev.ps1` 在**本机**起（独立项目/容器/卷/网络/端口 8092/密钥，本机可同时跑生产镜像那套 8080 与候选版本 8092）；② `deploy-dev.yml` 去掉部署 job，只做「测试 + 构建推 `:dev` 镜像」，并在 Summary 里给出本机那一行命令（Runner 在云上，到不了开发机）；③ 删掉只服务于"服务器上跑 dev 栈"的 `deploy/gh_deploy_dev.py`，服务器侧的 dev 栈与 8092 防火墙规则一并收回（数据卷保留待清理）。同一轮还把开发环境那句「迭代服务还没配置」改成**会自述原因**（新增 `Iteration:UnconfiguredHint`，默认空 → 生产行为一字不变）。 |
